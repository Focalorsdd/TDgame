...
public:
	void on_update(double delta)
	{
		...
		if (instance->is_game_over)
			return;
		if (!is_wave_started)
			timer_strat_wave.on_update(delta);
		else
		...
		if (is_spawned_last_enemy && EnemyManager::instance()->check_cleared())
		{
			...
			else
			{
				...
				timer_strat_wave.set_wait_time(wave.interval);
				timer_strat_wave.restart();
...
protected:
	WaveManager()
	{
		...
		timer_strat_wave.set_one_shot(true);
		timer_strat_wave.set_wait_time(wave_list[0].interval);
		timer_strat_wave.set_on_timeout([&]()
			{
